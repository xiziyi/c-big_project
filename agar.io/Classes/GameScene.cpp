...
bool Game::init()
{
	...
	gameListener->onTouchEnded = [this](Touch* touch, Event* event)
	{
		...
		if (player->getNumberOfRunningActions() == 0)
		{
			...
			this->setViewPointCenter(nodeLocation);
		}
		else
		{
	...
	//每0.01秒调用一次更新函数（指向生成地图移动函数）
	//this->schedule(schedule_selector(Game::update), 0.01);
...
{
this->setViewPointCenter(0.01f, player->getPosition());
}
